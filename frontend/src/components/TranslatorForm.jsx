...
const TranslatorForm = () => {
  ...
  const swapLanguages = () => {
    // simple, reliable swap using current state values
    ...
    <main className="flex-1 mt-10 px-4">
      <div className="flex flex-wrap justify-between gap-3 p-4 text-center">
        <div className="flex w-full flex-col gap-3 items-center">
          <p style={{ color: '#0f172a' }} className="dark:text-white text-4xl font-black leading-tight tracking-[-0.033em]">
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-end gap-4 px-4 py-3">
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium text-[#1a202c] dark:text-white" htmlFor="from-language">Translate From</label>
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm font-medium text-[#1a202c] dark:text-white" htmlFor="to-language">Translate To</label>
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-6 px-4">
        <div className="flex flex-col">
          <div className="relative flex flex-col min-w-40 flex-1">
            <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder={`Enter ${fromLanguage} text here...`} className="form-input ui-box flex w-full min-w-0 resize-none overflow-hidden text-[#111618] dark:text-white focus:outline-0 focus:ring-2 focus:ring-primary/50 bg-white dark:bg-gray-800 placeholder:text-gray-400 dark:placeholder:text-gray-500 text-base font-normal leading-normal" />
        ...
        <div className="flex flex-col">
          <div className="relative flex flex-col min-w-40 flex-1">
            <div className="w-full ui-box result-box">{translation || 'Translation will appear here...'}</div>
            <div className="control-top-right">
      ...
      {/* Centered translate action spanning both columns */}
      <div className="translator-actions">
        <button onClick={handleTranslate} disabled={loading || !inputText} className={`btn-primary ${loading || !inputText ? 'btn-disabled' : ''}`}>
